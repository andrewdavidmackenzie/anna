...
async fn replication_factor_change() {
    ...
    // PUT a key with default replication=1
    client
        .put("rep_change_key", "initial")
        .await
        .expect("PUT failed");

    // Verify routing returns 1 address (replication=1)
    ...
    // Routing should now return 2 addresses
...
async fn gossip_after_replication_change() {
    ...
    // Verify data is accessible (routing now returns both nodes)
