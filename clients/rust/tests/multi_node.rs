...
/// Test LWW merge semantics: concurrent writes to the same key from different
/// clients. After gossip convergence, the value with the higher timestamp wins.
///
/// Uses base_offset=800.
#[tokio::test]
#[cfg(unix)]
async fn lww_merge_last_writer_wins() {
    use annalib::kvs_client::KVSClient;
    ...
    let mut cluster = MultiNodeCluster::new(800);
    cluster.start_full_node(NODE1_IP, 2);
    cluster.start_kvs_node(NODE2_IP, NODE1_IP, 2);

    let config1 = cluster.client_config();
    let mut client1 = KVSClient::new(&config1, Some(43)).await;
    let mut client2 = KVSClient::new(&config1, Some(44)).await;

    // Write "first" then immediately overwrite with "second"
    client1
        .put("lww_merge_key", "first")
        .await
        .expect("PUT first failed");
    std::thread::sleep(Duration::from_millis(10));
    client2
        .put("lww_merge_key", "second")
        .await
    ...
    // Both clients should read "second" (higher timestamp wins)
    client1.clear_cache();
    client2.clear_cache();
    let v1 = client1.get("lww_merge_key").await.expect("GET failed");
    assert_eq!(v1, "second", "LWW should keep the later write");
}

/// Test Set merge semantics: concurrent set PUTs from different clients.
/// After gossip, the result should be the union of both sets.
///
/// Uses base_offset=900.
#[tokio::test]
#[cfg(unix)]
#[cfg(feature = "set")]
async fn set_merge_union() {
    use annalib::kvs_client::KVSClient;
    ...
    let mut cluster = MultiNodeCluster::new(900);
    cluster.start_full_node(NODE1_IP, 2);
    cluster.start_kvs_node(NODE2_IP, NODE1_IP, 2);

    let config = cluster.client_config();
    let mut client1 = KVSClient::new(&config, Some(45)).await;
    let mut client2 = KVSClient::new(&config, Some(46)).await;

    // Both clients add different elements to the same set
    client1
        .put_set("set_merge_key", &["a", "b"])
        .await
        .expect("PUT_SET 1 failed");
    client2
        .put_set("set_merge_key", &["c", "d"])
        .await
        .expect("PUT_SET 2 failed");

    // Wait for gossip to merge
    tokio::time::sleep(Duration::from_secs(TEST_GOSSIP_EPOCH as u64 + 2)).await;

    client1.clear_cache();
    let merged = client1
        .get_set("set_merge_key")
        .await
        .expect("GET_SET failed");
    assert!(
        merged.len() >= 2,
        "Set merge should contain elements from both writes, got {:?}",
        merged
    );
}

/// Test Priority merge semantics: concurrent writes with different priorities.
/// The value with the lowest priority should win.
///
/// Uses base_offset=1000.
#[tokio::test]
#[cfg(unix)]
async fn priority_merge_lowest_wins() {
    use annalib::kvs_client::KVSClient;
    ...
    let mut cluster = MultiNodeCluster::new(1000);
    cluster.start_full_node(NODE1_IP, 2);
    cluster.start_kvs_node(NODE2_IP, NODE1_IP, 2);

    let config = cluster.client_config();
    let mut client1 = KVSClient::new(&config, Some(47)).await;
    let mut client2 = KVSClient::new(&config, Some(48)).await;

    // Write with high priority, then low priority
    client1
        .put_priority("pri_merge_key", 10.0, "high_pri")
        .await
        .expect("PUT_PRIORITY high failed");
    client2
        .put_priority("pri_merge_key", 1.0, "low_pri")
        .await
    ...
    client1.clear_cache();
    let (priority, value) = client1
        .get_priority("pri_merge_key")
        .await
        .expect("GET_PRIORITY failed");
    assert!(
        priority <= 1.0,
        "Priority merge should keep lowest priority, got {}",
        priority
    );
    assert_eq!(value, "low_pri", "Should keep the low-priority value");
}
