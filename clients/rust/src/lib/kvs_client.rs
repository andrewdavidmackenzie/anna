...
pub struct KVSClient {
    ...
    /// High-water mark of timestamps generated by this client, ensuring
    /// each PUT uses a strictly increasing timestamp.
    last_write_ts: u64,
}
...
impl KVSClient {
    ...
    pub async fn put<K: AsRef<str> + Display>(&mut self, key: K, value: &str) -> Result<()> {
        debug!("PUT: {} <- {}", key, value);
        let ts = std::cmp::max(Self::generate_timestamp(), self.last_write_ts + 1);
